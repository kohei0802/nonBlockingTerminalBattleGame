...
void init_item_params(ITEM *p_win);
//initilaize boundary's position
void init_win_params(WIN *p_win);
//initialize NPC1's position
void init_npc1_params(NPC1 *p_win);
//initialize NPC2's position
void init_npc2_params(NPC2 *p_win);
//initialize Treasure's position
void init_treas_params(Treas *p_win);
//printing
void create_box(WIN *win, bool flag,ITEM *item,NPC1 *npc1,NPC2 *npc2,Treas *treas,int Coins, int HP,bool is_weapon);

//initialize player's position(boss)
void init_item_params_boss(ITEM *p_win);
//initialize boundary's position(boss)
void init_win_params_boss(WIN *p_win);
//initialize boss's position(boss)
void init_boss_params_boss(Boss *p_win);
//initialize spine(boss)
void init_spine_params_boss(Spine *spine_down, Spine *spine_up, Spine *spine_left, Spine *spine_right);
//printing(boss)
void create_boss(WIN *win,bool flag,ITEM *item,bool is_item,Boss *boss1,bool is_boss1,int wave_weapon,int HP, int boss_HP,Spine *spine_down,Spine *spine_up, Spine *spine_left,Spine *spine_right);

void npc1_action(int &HP, int &Coins, WIN *win)
{
	int ch;
	init_pair(4, COLOR_MAGENTA, COLOR_BLACK);
	mvprintw(win->starty + win->height + 2,win->startx,"Genie: I have a bottle of magic potion!");
	refresh();
	sleep(2.5);
	mvprintw(win->starty + win->height + 3,win->startx,"Genie: Do you want to try it!");
	refresh();
	sleep(2.5);
	mvprintw(win->starty + win->height + 4,win->startx,"Genie: It costs you 20 coins!  (y/n)");
	refresh();
	ch = getch();
	while(ch != 'y' and ch != 'n'){
		ch = getch();	
	...
	if(ch == 'y'){
		if(Coins < 20){
			...
			refresh();
			sleep(2.5);
			mvprintw(win->starty + win->height + 6,win->startx,"Genie: you can't pay for it, I never work for charity!");
			refresh();
			sleep(2.5);
			attron(COLOR_PAIR(4));
...
void treasure_action(bool &is_weapon,WIN *win)
{
	init_pair(4, COLOR_MAGENTA, COLOR_BLACK);
	attron(COLOR_PAIR(4));
	mvprintw(win->starty + win->height + 2,win->startx,"player1 opens the treasure!");
	attroff(COLOR_PAIR(4));
	refresh();
	sleep(2.5);
	if(is_weapon){
		attron(COLOR_PAIR(4));
		mvprintw(win->starty + win->height + 3,win->startx,"player1 finds nothing...");
		attroff(COLOR_PAIR(4));
		refresh();
	}
	else{
		attron(COLOR_PAIR(4));
		mvprintw(win->starty + win->height + 3,win->startx,"player1 finds his sword!!!");
		attroff(COLOR_PAIR(4));
		refresh();
		is_weapon = true;
	}
...
void npc2_action(bool &is_flash,WIN *win,int &Coins)
{	
	init_pair(4, COLOR_MAGENTA, COLOR_BLACK);
	if(is_flash == true){
	...
	refresh();
	sleep(2.5);
	mvprintw(win->starty + win->height + 3,win->startx,"master: 20 coins for tuition fee!  (y/n)!");
	refresh();
	ch = getch();
	while(ch != 'y' and ch != 'n'){
		ch = getch();	
	...
	if(ch == 'y'){
		if(Coins < 20){
			mvprintw(win->starty + win->height + 4,win->startx,"player1: Yes, my master!");
			refresh();
			sleep(2.5);
			...
			sleep(2.5);
			attron(COLOR_PAIR(4));
			mvprintw(win->starty + win->height + 5,win->startx,"player1 has gained the ability to freeze!!! press 'f' to freeze , press arrow keys to release freeze" );
			attron(COLOR_PAIR(4));
			refresh();
			is_flash = true;
			sleep(2.5);
...
//enter boss room
int boss_action(int &HP, bool is_flash)
{	
	timeout(0);
	int res = 0;
	WIN win;
...
//print boss room
void create_boss(WIN *p_win,bool flag,ITEM *item,bool is_item,Boss *boss1,bool is_boss1,int wave_weapon,int HP, int boss_HP,Spine *spine_down,Spine *spine_up,Spine *spine_left,Spine *spine_right)
{	
	int i, j;
	int x, y, w, h;
	...
	{	
		...
		if(item.starty == win.starty + win.height - 6 and item.startx == win.startx + win.width - 6){
			if(is_weapon == false){
				init_pair(4, COLOR_MAGENTA, COLOR_BLACK);
				attron(COLOR_PAIR(4));
				mvprintw(win.starty + win.height + 2,win.startx,"Please finding your sword!");
				attroff(COLOR_PAIR(4));
				refresh();
				sleep(2.5);
			}
...
void create_box(WIN *p_win, bool flag,ITEM *item,NPC1 *npc1 ,NPC2 *npc2,Treas *treas, int Coins, int HP,bool is_weapon)
{	
	int i, j;
	int x, y, w, h;
	...
	{	
		...
		mvprintw(npc1->starty+5, npc1->startx-1,"genie");
		attroff(COLOR_PAIR(3));
		//print npc2
		attron(COLOR_PAIR(3));
